...
def count_calls(method: Callable) -> Callable:
    """decorator that takes a single method Calable argument
...
def call_history(method: Callable) -> Callable:
    """stores the history of inputs and outputs for a particularfunction
    """
    @wraps(method)
    def wrapper(self, *args, **kwargs):
...
class Cache():
    ...
    def get_int(self, key: str) -> int:
        """ Transform a redis type variable toa str python type """
        variable = self._redis.get(key)
        try:
